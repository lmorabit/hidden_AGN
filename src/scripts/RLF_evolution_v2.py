...
simba_mycols = plt.cm.cividis(np.linspace(0,1,n))
...
## read in SIMBA and convert to same values
Simba = Table.read( paths.static / 'RLFs_flux_lim.csv', format='csv', delimiter=',' )
Simba['logP'] = Simba['logP'] + np.log10( np.power( (144./1400.), si ) )
for cn in ['SF','AGN','SFGs','Qs']:
    Simba[cn] = Simba[cn]+np.log10(2.5)

simba_z_bin_cens = np.unique(Simba['z'])
simba_z_bin_cens = simba_z_bin_cens[1:]
...
simba_zcols = simba_mycols[np.arange(0,len(z_lum_bins))*int(n/len(z_lum_bins))]
...
for i in np.arange(0,len(simba_z_bin_cens)):
    ...
    p3.plot( tmp_Simba['logP'][non_zero_idx], tmp_Simba['SFGs'][non_zero_idx], color=simba_zcols[i], linewidth=2.5, linestyle='dotted' )
    p3.plot( tmp_Simba['logP'][non_zero_idx], tmp_Simba['SF'][non_zero_idx], color=simba_zcols[i], linewidth=2.5, label='{:s} < z < {:s}'.format(str(zbin_starts[i]),str(zbin_ends[i])) )
...
dline = matplotlib.lines.Line2D([0],[0], color='black', linewidth=3 )
lline = matplotlib.lines.Line2D([0],[0], color='black', linewidth=3, linestyle='dotted' )
l2 = p3.legend((lline,dline),('Galaxies', 'Activity'),loc='upper right')
p3.add_artist(l1)
...
for i in np.arange(0,len(simba_z_bin_cens)):
    ...
    tmp_Simba = Simba[z_idx]
    non_zero_idx = np.where( np.logical_and( np.isfinite(tmp_Simba['SF']), np.isfinite(tmp_Simba['SFGs']) ) )[0]
    ratio = np.power(10., tmp_Simba['SF'][non_zero_idx] ) / np.power( 10., tmp_Simba['SFGs'][non_zero_idx] )
